...
for kk=1:size(folder_challenges,1)
    for y=1:size(folders_categories{kk},1)
        ...
        images_per_category = round(images_per_category);  % 将其四舍五入为整数
        images_per_category = images_per_category(1);  % 取向量的第一个元素，确保为标量整数
        number_sampling_images = max(0, round(number_sampling_images(1)));  % 取其四舍五入后的最大值，确保为非负整数
        disp(number_sampling_images);
